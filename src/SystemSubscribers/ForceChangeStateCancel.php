...
class ForceChangeStateCancel extends Job implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if($this->complaint->state->short_code == 'pending'){
            if($this->complaint->update(['status' => $status = sys('model.status')->where('short_code', '=', 'discard')->first()->id])){
                //send email to user and staff
                Log::info('Complaint#'. $this->complaint->complain_no .' status changed to discard from pending.');
                ...
                Log::info('Complaint#'. $this->complaint->complain_no .' failed to change status to discard.');
            }
        }else{
            Log::info('Complaint#'. $this->complaint->complain_no .' status is not pending, nothing to do.');
        }
    }
}
